...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!service) return { title: "Program not found" };
  return {
    title: service.name,
    description: service.description,
    openGraph: {
      title: `${service.name} | GrindLine Skate Academy`,
      description: service.description,
...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  return (
    <>
          ...
          <Reveal>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/contact" className={cn(buttonVariants({ size: "lg" }), "btn-gradient rounded-full")}>
                Book This Program <ArrowRight className="size-4" />
              </Link>
              <Link
                href="tel:+15550158890"
                className={cn(
      ...
      <section className="px-4 py-14 sm:py-16">
        <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[1fr_20rem]">
          ...
          <div className="space-y-6">
            <Reveal delay={0.05}>
              <div className="glass-card rounded-3xl p-7">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Every skater is different. Tell us your goals and we will recommend the right
                  session type, coach and schedule — with no obligation.
                </p>
                <Link
                  href="/contact"
            ...
            <Reveal delay={0.1}>
              <div className="glass-card rounded-3xl p-7">
                <h3 className="font-display text-base font-bold text-foreground">Gear & location</h3>
                <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                  We are at 88 Rail Yard Ave, Portland — with covered and indoor terrain, so we
                  skate rain or shine. Boards, helmets and pads are free with every session.
                </p>
              </div>
            </Reveal>
          </div>
      ...
      <CtaBand
        title={`Ready to try ${service.name.toLowerCase()}?`}
        subtitle="Tell us about your experience level and goals — a coach will match you with the perfect session and respond within one business day."
      />
    </>
  );
}
